...
| Layer | Purpose | Tools |
|-------|---------|-------|
| **Layer 1: Math Capability** | Does SFT/RL improve math correctness? | `evaluate_math_model.py` |
...
```bash
# Full evaluation (base + SFT + GRPO on GSM8K)
bash evaluation/run_full_eval.sh

# Quick smoke test (30 examples)
...
python evaluation/evaluate_math_model.py \
  ...
  --max-new-tokens 1024 \
  --prompt-style xml \
  --output evaluation/results/base_gsm8k_xml_1024_300.json \
  --save-outputs
...
python evaluation/evaluate_math_model.py \
  ...
  --max-new-tokens 1024 \
  --prompt-style xml \
  --output evaluation/results/sft_gsm8k_xml_1024_300.json \
  --save-outputs
...
python evaluation/evaluate_math_model.py \
  ...
  --max-new-tokens 1024 \
  --prompt-style xml \
  --output evaluation/results/grpo_gsm8k_xml_1024_300.json \
  --save-outputs
```
...
python evaluation/evaluate_math_model.py \
  ...
  --output evaluation/results/grpo_gsm8k_xml_1024_300_passk.json \
  --save-outputs
```

### 2. `compare_results.py` — Multi-model Comparison
...
python evaluation/compare_results.py \
  --inputs \
    ...
    evaluation/results/grpo_gsm8k_xml_1024_300.json \
  --labels base sft grpo \
  --output-json evaluation/results/gsm8k_comparison.json \
  --output-md evaluation/results/gsm8k_comparison.md \
  --show-by-difficulty \
  --show-by-topic
```
...
1. **Sensitivity to data filtering?**
...
- Default `--limit` is 300 for statistical significance. Use 30 for quick debugging.
